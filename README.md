...
候補は [`src/lib/models.ts`](src/lib/models.ts)（フロント）と [`worker/lib/models.ts`](worker/lib/models.ts)（バックエンドの許可リスト）で定義:

- `openai/gpt-5.5`
- `openai/gpt-5.4-nano`

> 接続先は現状 Perplexity エンドポイント（`PERPLEXITY_API_KEY`）です。Perplexity が上記モデルIDを受け付けない場合は
> [`worker/lib/perplexity.ts`](worker/lib/perplexity.ts) の base URL を OpenRouter（`https://openrouter.ai/api/v1`）等へ変更してください。
