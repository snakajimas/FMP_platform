...
### AIモデル選択（Perplexity Agent API 経由）

AI は **Perplexity の Agent API**（`POST https://api.perplexity.ai/v1/agent`）を使います。
**Perplexity キー1つ**で `openai/*` 等の `provider/model` を呼べるのが特長です（OpenAI のキーは不要）。
クライアント実装は [`worker/lib/llm.ts`](worker/lib/llm.ts)。

ヘッダー右上のセレクタで使用モデルを切り替えられます（チャット・スクリーナー共通、localStorage 保存）。
候補は [`src/lib/models.ts`](src/lib/models.ts)（フロント）と [`worker/lib/models.ts`](worker/lib/models.ts)（バックエンド許可リスト）で定義:

- `openai/gpt-5.5`
- `openai/gpt-5.4-nano`

> Agent API はリクエストが `{ model, input }`（`input` は文字列）、レスポンス本文は `output_text` です。
> 利用可能なモデル一覧は `GET https://api.perplexity.ai/v1/models` で確認できます。
> モデルを増やす場合は上記2ファイルの一覧に id を追記してください。
